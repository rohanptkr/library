...
public class MemberController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if ("new".equals(action)) {
                ...
                // Handle renew membership page - load all active members initially
                List<Member> activeMembers = memberDAO.getAllActiveMembers();
                request.setAttribute("activeMembers", activeMembers);
    ...
    private void searchMembers(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (searchQuery != null && !searchQuery.trim().isEmpty()) {
            // Use the search method to find matching members
            List<Member> searchResults = memberDAO.searchMembers(searchQuery.trim());
            request.setAttribute("activeMembers", searchResults);
            request.setAttribute("searchQuery", searchQuery);
            if (searchResults.isEmpty()) {
                request.setAttribute("message", "No members found matching: '" + searchQuery + "'");
            } else {
                request.setAttribute("message", "Found " + searchResults.size() + " member(s) matching: '" + searchQuery + "'");
            }
        } else {
    ...
    private void registerMember(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (success) {
                // Get the latest member details to display (including the generated member ID)
                List<Member> latestMembers = memberDAO.getAllActiveMembers();
                Member registeredMember = latestMembers.stream()
